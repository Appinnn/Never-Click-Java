...
public class GenericStudy1 
{
	...
	public static void main(String[] args) 
	{
		...
		// 사과상자 객체 꺼내오기(제네릭을 사용했기 때문에, 형변환 필요x)
		Apple apple = appleBox.getFruit();
		
		
		// 제네릭 T가 fruitBox와 상속을 맺었기 때문에 Toy클래스는 들어오지 못한다.
...
/*
	...
	- 제네릭 타입으로 사용 가능한 클래스 제한을 두는 방법
		class A<T extends Parent>   -> Parent 클래스의 자식 클래스와 Parent 클래스만 제네릭으로 타입 지정이 가능하다.
		 이외 다른 클래스들은 불가능.
		 -> 다중 타입으로 제한을 설정 할 수 있다.(다중상속 가능)
		 	인터페이스만 다중 타입 설정 가능, 클래스는 안됨
		 	
	- 제네릭 와일드 카드 <?>
	- 제네릭 타입의 한계를 보완하기 위해 사용하는 방법
	- 메서드의 매개변수 타입으로 제네릭 타입 표현되는 클래스를 사용한다면, 클래스에 표현된 제네릭이 제대로 표현되지 않기 때문에
		다시한번 제네릭 지정을 해야 한다. 이때 사용되는 와일드카드<?> 는 예외없이 허용 한다 라는 표현 방식
		
	- 와일드 카드의 제한
		<? extends T> : T 클래스와 그 자식까지 가능
		<? super T> : T 클래스와 그 조상까지 가능(자식클래스 사용 불가)
