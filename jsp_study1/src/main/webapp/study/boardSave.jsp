...
<%
	...
	// 이미지 업로드해서 저장될 실제 위치
	String path="C:\\LSM\\Never-Click-Java\\jsp_study1\\src\\main\\webapp\\study\\image";
	//업로드할 이미지의 최대크기
	...
	try
	{
		...
		String file = (String)em.nextElement(); // 이미지 한개만 업로드 하기때문에 nextElement() 한번만 하면 된다.
		imgName = mr.getFilesystemName(file);   // 실제 저장된 파일이름
		// 원본이름을 가지고 오려면 - mr.getOriginalFileName(file);
	...
	// db.pt = db.com.prepareStatement(sql);
	...
	int boardId =0; // 방금 저장 한 게시글 번호를 저장하기 위한 변수
