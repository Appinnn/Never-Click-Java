...
<%--
	 ...
	 id 파라미터의 값을 board테이블의 board_id 컬럼으로 조회하여
...
<%
	...
	Board data = null; // select sql문의 결과는 Board 클래스 객체 하나에 저장되는 데이터
	 // 이기 때문에 Board클래스 객체 저장할 수 있는 변수만 있으면 된다.
	try
	{
		...
		{
			data = new Board(
					db.rs.getInt("board_id"), 
					db.rs.getString("writer"), 
